...
class MainFrame:
    ...
    def testMain(self):
        ...
        btnParamsScript = Button(self.tab2, text='1、根据配置参数生成脚本', command=self.getParamsScript, width=20)
        ...
        btnDevice = Button(self.tab2, text='2、自定义monkey脚本文件', command=self.get_monkey_path, width=20)
        btnDevice.grid(row=2, column=0, sticky=W, padx=10, pady=10)

        fram = Frame(self.tab2, bg="white")
        fram.grid(row=2, column=1, sticky=W, padx=10, pady=0)

        #monkey文件
        self.monkeyPath = Entry(fram, width=50,relief="groove",borderwidth=2)
        self.monkeyPath.grid(row=1, column=0, sticky=W)

        runcountLable = Label(fram, text='执行次数：' , bg="white")
        runcountLable.grid(row=1, column=1, sticky=E, padx=10, pady=0)

        self.runcount = Entry(fram, width=20, relief="groove", borderwidth=2)
        self.runcount.grid(row=1, column=2, sticky=W, padx=10)
        self.runcount.delete(0,END)
        self.runcount.insert(0,1)

        fram = Frame(self.tab2, bg="white")
        fram.grid(row=3, column=0, sticky=W, padx=10, pady=10, columnspan=2 )

        btnParamsScript = Button(fram, text='测试信息：', width=20)
        btnParamsScript.grid(row=1, column=0, sticky=W,  pady=10)

        self.testPerson = Entry(fram, width=20, relief="groove", borderwidth=2)
        self.testPerson.grid(row=1, column=1, sticky=W,padx=10)
        self.testPerson.delete(0, END)
        self.testPerson.insert(0, "测试人员01")

        self.modulename = Entry(fram, width=20, relief="groove", borderwidth=2)
        self.modulename.grid(row=1, column=2, sticky=W,padx=10)
        self.modulename.delete(0, END)
        self.modulename.insert(0, "健康一体机")

        self.apkversion = Entry(fram, width=20, relief="groove", borderwidth=2)
        self.apkversion.grid(row=1, column=3, sticky=W,padx=10)
        self.apkversion.delete(0, END)
        self.apkversion.insert(0, "V1.0.0.480")

        fram = Frame( self.tab2,bg="white" )
        fram.grid(row=4, column=0, sticky=W, padx=10, pady=0 , columnspan=2)

        btnDevice = Button(fram, text='运行1', command=self.runOneMonkeyScript, width=20)
        btnDevice.grid(row=1, column=0, sticky=W, padx=0, pady=0)

        btnDevice = Button(fram, text='运行2', command=self.runTwoMonkeyScript, width=20)
        btnDevice.grid(row=1, column=1, sticky=W, padx=10, pady=0)

        btnDevice = Button(fram, text='打开测试报告目录', command=self.openMonkeyTestReport, width=20)
        btnDevice.grid(row=1, column=2, sticky=W, padx=10, pady=0)

        btnDevice = Button(fram, text='清空日志', command=self.clearLog, width=20)
        btnDevice.grid(row=1, column=3, sticky=W, padx=10, pady=0)

        # 文本域
        self.text = ScrolledText(self.tab2, width=108,height=25 , relief="groove",borderwidth=2)
        self.text.configure(state=tkinter.DISABLED)
        self.text.grid(row=5, column=0, sticky=W, padx=10, pady=10, columnspan=2)
